...
Every tool call against your own context requires an OAuth token carrying the matching scope, so a connection can be narrowed to exactly what a client needs. The one read that needs no token is a profile its owner has set to public, addressed by its @handle. The scopes and the rest of the access model are covered in [Security](https://usemycontext.ai/docs/security).
