...
- Without authentication the MCP server serves exactly two things: static tool metadata (names, descriptions, schemas) for public discovery, and the public context behind an @handle, for a profile whose owner has deliberately set it to public. The second is how an AI client reads someone's public page without an account of its own. Everything else requires a valid token: every other tool, and every profile that is not public.
